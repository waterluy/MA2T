...
class VADCustomNuScenesDataset(NuScenesDataset):
    ...
    def _evaluate_single(self,
        ...
        self.nusc_eval.main(plot_examples=0, render_curves=False, jsonfile_prefix=jsonfile_prefix)
        # record metrics
        ...
        for metric in map_metrics:
            ...
            metrics_folder = osp.join(jsonfile_prefix, 'metrics', )
            with open(osp.join(metrics_folder, 'map.csv'), mode='w', newline='') as file:
                fieldnames = ['Class', 'AP']
                writer = csv.DictWriter(file, fieldnames=fieldnames)
                writer.writeheader()
                for i, name in enumerate(self.MAPCLASSES):
                    writer.writerow({'Class': name, 'AP': '%.4f' % cls_aps.mean(0)[i]})
                writer.writerow({'Class': 'map', 'AP': '%.4f' % cls_aps.mean(0).mean()})
    ...
    def evaluate(self,
        ...
        metrics_folder = osp.join(jsonfile_prefix, 'metrics', )
        mmcv.mkdir_or_exist(metrics_folder)
        csv_rows = []
        with open(osp.join(metrics_folder, 'motion.csv'), mode='w', newline='') as file:
            # 定义 CSV 文件的列名
            fieldnames = ['class-names', 'min_ade_err', 'min_fde_err', 'miss_rate_err', 'epa']
            # 创建 DictWriter 对象
            writer = csv.DictWriter(file, fieldnames=fieldnames)
            # 写入表头
            writer.writeheader()
            ...
            writer.writerow({
                'class-names': 'vehicle',
                'min_ade_err': '%.4f' % result_dict['ADE_car'].item() if isinstance(result_dict['ADE_car'], torch.Tensor) else result_dict['ADE_car'],
                'min_fde_err': '%.4f' % result_dict['FDE_car'].item() if isinstance(result_dict['FDE_car'], torch.Tensor) else result_dict['FDE_car'],
                'miss_rate_err': '%.4f' % result_dict['MR_car'],
                'epa': '%.4f' % result_dict['EPA_car']
            })
            # 写入行人数据
            writer.writerow({
                'class-names': 'pedestrian',
                'min_ade_err': '%.4f' % result_dict['ADE_pedestrian'].item() if isinstance(result_dict['ADE_pedestrian'], torch.Tensor) else result_dict['ADE_pedestrian'],
                'min_fde_err': '%.4f' % result_dict['FDE_pedestrian'].item() if isinstance(result_dict['ADE_pedestrian'], torch.Tensor) else result_dict['ADE_pedestrian'],
                'miss_rate_err': '%.4f' % result_dict['MR_pedestrian'],
                'epa': '%.4f' % result_dict['EPA_pedestrian']
            })
        ...
        fieldnames = ['metrics', '1.0s', '', '2.0s', '3.0s', 'avg']
        data = [
            {
                'metrics': 'obj_col',
                '1.0s': round(metric_dict['plan_obj_col_1s'], 4),
                '2.0s': round(metric_dict['plan_obj_col_2s'], 4),
                '3.0s': round(metric_dict['plan_obj_col_3s'], 4),
                'avg': calculate_avg([
            ...
            {
                'metrics': 'obj_box_col',
                '1.0s': round(metric_dict['plan_obj_box_col_1s'], 4),
                '2.0s': round(metric_dict['plan_obj_box_col_2s'], 4),
                '3.0s': round(metric_dict['plan_obj_box_col_3s'], 4),
                'avg': calculate_avg([
            ...
            {
                'metrics': 'L2',
                '1.0s': round(metric_dict['plan_L2_1s'], 4),
                '2.0s': round(metric_dict['plan_L2_2s'], 4),
                '3.0s': round(metric_dict['plan_L2_3s'], 4),
                'avg': calculate_avg([
        ...
        with open(osp.join(metrics_folder, 'planning.csv'), mode='w', newline='') as file:
            writer = csv.DictWriter(file, fieldnames=fieldnames)
            writer.writeheader()
            # 写入数据
            for row in data:
        ...
        if isinstance(result_files, dict):
            ...
            for name in result_names:
                ...
                ret_dict = self._evaluate_single(result_files[name], metric=metric, map_metric=map_metric, jsonfile_prefix=jsonfile_prefix)
            results_dict.update(ret_dict)
        elif isinstance(result_files, str):
            results_dict = self._evaluate_single(result_files, metric=metric, map_metric=map_metric, jsonfile_prefix=jsonfile_prefix)
...
def output_to_vecs(detection):
    box3d = detection['map_boxes_3d'].detach().numpy()
    scores = detection['map_scores_3d'].detach().numpy()
    labels = detection['map_labels_3d'].detach().numpy()
    pts = detection['map_pts_3d'].detach().numpy()
