...
class VADHead(DETRHead):
    ...
    @property
    def attack_map(self):
        return self.adv.attack_map
    
    @property
    def attack_agent(self):
        return self.adv.attack_agent

    @property
    def noise_map(self):
        return self.adv.noise_map
    
    @property
    def noise_agent(self):
        return self.adv.noise_agent
    ...
    def forward(self, *args, **kwargs):
        if self.adv_train or self.attack:
            return self.forward_adv(*args, **kwargs)
        else:
    ...
    def forward_adv(self,
        ...
        """
        # 只有 mlvl_feats 是当前图像的特征，有梯度追踪， prev_bev 无梯度追踪
...
# agent_query 和 motion_hs 关联
# map_query 和 map_hs 关联
        # print(motion_hs.shape)  # torch.Size([1, 300, 6, 512])
        # print(map_hs.shape) # torch.Size([6, 1, 2000, 256])
        # planning
        (batch, num_agent) = motion_hs.shape[:2]
        if self.ego_his_encoder is not None:    # 都是 None
        ...
        # print(agent_query.shape)    # torch.Size([1, 300, 3072])
        if self.attack_agent:
            agent_query = agent_query + self.noise_agent
        ...
        # print(map_query.shape)  # torch.Size([1, 100, 20, 256])
        if self.attack_map:
            map_query = map_query + self.noise_map
        ...
        outs = {
            'bev_embed': bev_embed,
            'all_cls_scores': outputs_classes,  # 只用作 分数 筛选， grad 断       计算损失
            'all_bbox_preds': outputs_coords,   # 只用来直接计算损失
            'all_traj_preds': outputs_trajs.repeat(outputs_coords.shape[0], 1, 1, 1, 1),    # 只用来直接计算损失
            'all_traj_cls_scores': outputs_trajs_classes.repeat(outputs_coords.shape[0], 1, 1, 1),  # 只用来直接计算损失
            'map_all_cls_scores': map_outputs_classes,  # 用作 分数 筛选， grad 断，      计算损失
            'map_all_bbox_preds': map_outputs_coords,   # 只用来直接计算损失
            'map_all_pts_preds': map_outputs_pts_coords,    # 只用来直接计算损失
            'enc_cls_scores': None,
            'enc_bbox_preds': None,
            'map_enc_cls_scores': None,
            'map_enc_bbox_preds': None,
            'map_enc_pts_preds': None,
            'ego_fut_preds': outputs_ego_trajs, # 最终结果
        }
