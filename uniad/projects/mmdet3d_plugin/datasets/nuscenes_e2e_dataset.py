...
class NuScenesE2EDataset(NuScenesDataset):
    ...
    def prepare_train_data(self, index):
        ...
        if self.enbale_temporal_aug:
            ...
            if first_index < 0:
                # print('line 197: first_index < 0', 'index: ', index, 'queue_length: ', self.queue_length)
                return None
            if self.data_infos[first_index]['scene_token'] != \
                    self.data_infos[final_index]['scene_token']:
                # print("line 201: scene_token != ", 'first: ', self.data_infos[first_index]['scene_token'], 'final: ', self.data_infos[final_index]['scene_token'])
                return None
            # current timestamp
    ...
    def prepare_attack_data(self, index):
        """
        Training data preparation.
        Args:
            index (int): Index for accessing the target data.
        ...
        if first_index < 0:
            # print('1, ', first_index, final_index, self.queue_length)
            return None
        if self.data_infos[first_index]['scene_token'] != \
                    self.data_infos[final_index]['scene_token']:
            # print('2, ', self.data_infos[first_index]['scene_token'], self.data_infos[final_index]['scene_token'])
            return None
        # current timestamp
        input_dict = self.get_data_info(final_index)
        prev_indexs_list = list(reversed(range(first_index, final_index)))
        assert 0 == len(prev_indexs_list)
        if input_dict is None:
            # print('3, ', input_dict)
            return None
        frame_idx = input_dict['frame_idx']
        ...
        if self.filter_empty_gt and \
                (example is None or ~(example['gt_labels_3d']._data != -1).any()):
            # print('4, ????????????', example['gt_labels_3d']._data) 
            # 这里会return None, example['gt_labels_3d']._data是tensor([], dtype=torch.int64)
            return None
        data_queue.insert(0, example)
        ...
        for i in prev_indexs_list:
            ...
            assert example['gt_labels_3d'].data.shape[0] == example['gt_past_traj'].shape[0]
            data_queue.insert(0, copy.deepcopy(example))
        data_queue = self.union2one_attack(data_queue)
        return data_queue
    
    def union2one_attack(self, queue):
        """
    ...
    def __getitem__(self, idx):
        ...
        if self.attack_mode:
            while True:
                data = self.prepare_attack_data(idx)
                if data is None:
    ...
    def evaluate(self,
        ...
        if isinstance(results, dict):
            if 'occ_results_computed' in results.keys():
                ...
                for panoptic_key in out_metrics:
                    print(panoptic_key)
                    csv_header.append(f'{panoptic_key}-n')
                    csv_header.append(f'{panoptic_key}-f')
                    # HERE!! connect
                    # print(' & '.join(
                    #     [f'{x:.1f}' for x in occ_results_computed[panoptic_key]]))
                    data = [f'{x:.1f}' for x in occ_results_computed[panoptic_key]]
                    print(' & '.join(data))
                    csv_row[f'{panoptic_key}-n'] = data[0]
                    csv_row[f'{panoptic_key}-f'] = data[1]
                with open(osp.join(metrics_folder, 'occ.csv', ), mode='w', newline='') as f:
            ...
            if 'planning_results_computed' in results.keys():
                ...
                csv_header = ["metrics", "0.5s", "1.0s", "1.5s", "2.0s", "2.5s", "3.0s", "avg"]
                csv_rows = []
                for key in planning_results_computed.keys():
                    ...
                    csv_row[csv_header[ttt]] = key
                    ttt += 1 
                    for i in range(len(value)):
                        row_value.append('%.4f' % float(value[i]))
                        csv_row[csv_header[ttt]] = '%.4f' % float(value[i])
                        ttt += 1
                    csv_row["avg"] = (float(csv_row["1.0s"]) + float(csv_row["2.0s"]) + float(csv_row["3.0s"])) / 3
                    planning_tab.add_row(row_value)
                    csv_rows.append(csv_row)
                print(planning_tab)
                with open(osp.join(metrics_folder, 'planning.csv', ), mode='w', newline='') as f:
                    writer = csv.DictWriter(f, fieldnames=csv_header)
                    writer.writeheader()
        ...
        if isinstance(result_files, dict):
            ...
            for name in result_names:
                ...
                ret_dict = self._evaluate_single(
                    result_files[name], result_files_det[name], jsonfile_prefix=jsonfile_prefix)
            results_dict.update(ret_dict)
        elif isinstance(result_files, str):
            results_dict = self._evaluate_single(
                result_files, result_files_det, jsonfile_prefix=jsonfile_prefix)
        ...
        if 'map' in self.eval_mod:
            ...
            iou_data = {'drivable_iou': float(drivable_intersection / drivable_union),
            ...
            results_dict.update(iou_data)

            print(results_dict)
            ###### save map metrics ######
            metrics_folder = osp.join(jsonfile_prefix, 'metrics', )
            mmcv.mkdir_or_exist(metrics_folder)
            csv_header = ['drivable_iou', 'lanes_iou', 'divider_iou', 'crossing_iou', 'contour_iou']
            with open(osp.join(metrics_folder, 'map.csv', ), mode='w', newline='') as f:
                    writer = csv.DictWriter(f, fieldnames=csv_header)
                    writer.writeheader()
                    writer.writerow(iou_data)
    ...
    def _evaluate_single(self,
        ...
        if 'det' in self.eval_mod:
            ...
            self.nusc_eval.main(plot_examples=0, render_curves=False)   # render_curves是否绘制pr图
            # record metrics
            metrics = mmcv.load(
                osp.join(
                    output_dir_det,
        ...
        if 'track' in self.eval_mod:
            ...
            metrics_folder = osp.join(jsonfile_prefix, 'metrics', )
            mmcv.mkdir_or_exist(metrics_folder)
            csv_header = ['amota', 'amotp', 'recall', 'ids',]
            csv_row = {}
            for k in csv_header:
                csv_row[k] = metrics[k]
            import csv
            with open(osp.join(metrics_folder, 'track.csv', ), mode='w', newline='') as f:
                    writer = csv.DictWriter(f, fieldnames=csv_header)
                    writer.writeheader()
                    writer.writerow(csv_row)
        ...
        if 'motion' in self.eval_mod:
            ...
            print('-'*50)
            print(
                'Motion Evaluate on motion category, merge class for vehicles and pedestrians...')
            print('evaluate standard motion metrics...')
            self.nusc_eval_motion.main(
                plot_examples=1,
                render_curves=False,    # True绘制pr曲线
                eval_mode='standard')
            ...
            self.nusc_eval_motion.main(
                ...
                eval_mode='epa')
            print('-'*50)
            # print('Motion Evaluate on detection category...')
            # self.nusc_eval_motion = MotionEval(
            #     self.nusc,
            #     config=self.eval_detection_configs,
            #     result_path=result_path,
