...
class MotionEval(NuScenesEval):
    ...
    def __init__(self,
        ...
        self.category_convert_type = category_convert_type
    ...
    def evaluate(self) -> Tuple[DetectionMotionMetrics,
        ...
        if 'motion_category' == self.category_convert_type:
            print_traj_metrics(traj_metrics, tocsv=True, jsonfile_prefix=self.jsonfile_prefix)
        else:
    ...
    def evaluate_epa(
        ...
        if 'motion_category' == self.category_convert_type:
            import csv
            metrics_folder = osp.join(self.jsonfile_prefix, 'metrics', )
            csv_rows = []
            with open(osp.join(metrics_folder, 'motion.csv', ), mode='r') as f:
                reader = csv.DictReader(f)
                csv_header = list(reader.fieldnames)
                for l in reader:
                    csv_rows.append(dict(l))
            csv_header.append("epa")
        ...
        for class_name in self.cfg.class_names:
            for dist_th in self.cfg.dist_ths:
                ...
                if 'motion_category' == self.category_convert_type:
                    if 'car' == class_name:
                        for d in csv_rows:
                            if "vehicle" == d["class-names"]:
                                d["epa"] = EPA
                    else:
                        for d in csv_rows:
                            if class_name == d["class-names"]:
                                d["epa"] = EPA
        if 'motion_category' == self.category_convert_type:
            with open(osp.join(metrics_folder, 'motion.csv', ), mode='w', newline='') as f:
...
def print_traj_metrics(metrics, tocsv=False, jsonfile_prefix=None):
    ...
    if tocsv:
        ...
        csv_rows = []
        csv_header = ["class-names"] + TP_TRAJ_METRICS
    for class_name in metrics.keys():
        row_data = [class_name]
        csv_row = {}
        if tocsv and ("car" == class_name):
            csv_row["class-names"] = "vehicle"
        if tocsv and ("pedestrian" == class_name):
            csv_row["class-names"] = "pedestrian"
        for m in TP_TRAJ_METRICS:
            row_data.append('%.4f' % metrics[class_name][m])
            if tocsv and (("car" == class_name) or ("pedestrian" == class_name)):
                csv_row[m] = '%.4f' % metrics[class_name][m]
        if tocsv and (("car" == class_name) or ("pedestrian" == class_name)):
