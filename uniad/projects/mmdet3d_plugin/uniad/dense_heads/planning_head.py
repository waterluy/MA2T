...
class PlanningHeadSingleMode(nn.Module):
    ...
    def forward_attack(self,
        ...
        bev_pos = outs_motion['bev_pos']
        # occ_mask = None TODO
        if outs_occflow != None:
            occ_mask = outs_occflow['seg_out']
        else:
            occ_mask = None
        outs_planning, outs_planning_for_loss = self(bev_embed, occ_mask, bev_pos, sdc_traj_query, sdc_track_query, command, attack=True)
        # outs_planning_for_loss = self(bev_embed, None, bev_pos, sdc_traj_query, sdc_track_query, command)
        loss_inputs = [sdc_planning, sdc_planning_mask, outs_planning_for_loss, gt_future_boxes]
        losses = self.loss(*loss_inputs)
        ret_dict = dict(losses=losses, )
        return ret_dict, outs_planning
    ...
    def forward(self, 
        ...
        if attack:
            sdc_traj_all_for_loss = sdc_traj_all.clone()
        ...
        if attack:
            return dict(sdc_traj=sdc_traj_all, sdc_traj_all=sdc_traj_all,), dict(sdc_traj=sdc_traj_all_for_loss, sdc_traj_all=sdc_traj_all_for_loss,)
        else:
            return dict(
