...
class PansegformerHead(SegDETRHead):
    ...
    def forward_test(self,
        ...
        with torch.no_grad():
            ...
            # gt_lane_masks[0].shape  1,12,200,200
    ...
    # forward_train和forward_test都只用到了bev_feat
    # 二者都是先经过了forward
    # train输出是loss+原始forward输出
    # test输出是加工后的原始forward输出
    # 但是在下一个motion阶段都只用到了seg输出的args_tuple

    # 数据符合forward_train
    def forward_attack(self, 
                        bev_feat=None,              # pts_feats=None,
                        img_metas=None,
                        gt_lane_labels=None,
                        gt_lane_bboxes=None,
                        gt_lane_masks=None,
        ...
        return losses_seg, pred_seg_dict, bbox_list    
        # pred_seg_dict 是train阶段用的输出
        # 但是motion阶段只用了args_tuple

    # 数据符合test数据
    def forward_attack_test(self, 
                        bev_feat=None,              # pts_feats=None,
                        img_metas=None,
                        gt_lane_labels=None,
                        gt_lane_bboxes=None,
