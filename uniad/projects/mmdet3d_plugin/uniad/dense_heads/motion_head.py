...
class MotionHead(BaseMotionHead):
    ...
    def forward_train(self,
        ...
        """
        # outs_track['track_query_embeddings'].shape : torch.Size([37, 256]) 有grad_fn
        # track_query.shape : torch.Size([1, 1, 37, 256])   有grad_fn
        track_query = outs_track['track_query_embeddings'][None, None, ...] # num_dec, B, A_track, D
        # outs_track['track_query_matched_idxes'].shape: torch.Size([37]), 无梯度 cpu
        all_matched_idxes = [outs_track['track_query_matched_idxes']] #BxN
        # outs_track['track_bbox_results'][0]有5个元素 无梯度 cpu
        ...
        # outs_track['sdc_embedding'] size256 有grad_fn
        # print(track_query.shape)
    ...
    def forward_attack(self,
        ...
        '''
        # test
        def filter_vehicle_query(outs_motion, labels, vehicle_id_list, all_matched_idxes):
            ...
            all_matched_idxes[0] = all_matched_idxes[0][vehicle_mask>0]
            return outs_motion, all_matched_idxes
        outs_motion, all_matched_idxes = filter_vehicle_query(outs_motion, labels, self.vehicle_id_list, all_matched_idxes)
        # filter sdc query
        ...
        ret_dict = dict(losses=losses, outs_motion=outs_motion, track_boxes=track_boxes)
        # outs_motion是用于occ阶段的, traj_results是用于metrics和vis的
        return ret_dict, traj_results
    ...
    def forward(self, 
        ...
        # track_query.shape:   1, 1, 39, 256        39不固定
