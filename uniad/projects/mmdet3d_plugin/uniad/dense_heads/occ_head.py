...
class OccHead(BaseModule):
    ...
    def merge_queries(self, outs_dict, detach_query_pos=True):
        ins_query = outs_dict.get('traj_query', None)       # [n_dec, b, nq, n_modes, dim] n_dec:number of transformer decoder layers
        track_query = outs_dict['track_query']              # [b, nq, d]
        track_query_pos = outs_dict['track_query_pos']      # [b, nq, d]
        
        assert True == detach_query_pos
        if detach_query_pos:    # 是True
            track_query_pos = track_query_pos.detach()

        # print(ins_query.shape)
    ...
    def forward_train(
        ...
        for ind in range(bs):
            ...
            loss_dice = loss_dice + cur_dice_loss
            loss_mask = loss_mask + cur_mask_loss
            loss_aux_dice = loss_aux_dice + cur_aux_dice_loss * self.aux_loss_weight
            loss_aux_mask = loss_aux_mask +  cur_aux_mask_loss * self.aux_loss_weight
    ...
    def forward_attack(
        ...
        for param in self.parameters():
            param.requires_grad = False
        # Generate warpped gt and related inputs
        gt_segmentation, gt_instance, gt_img_is_valid = self.get_occ_labels_attack(gt_segmentation, gt_instance, gt_img_is_valid)
        ...
        # test
        # pred_ins_logits = copy.deepcopy(ins_seg_preds_batch)
        pred_ins_logits = ins_seg_preds_batch.clone() # .detach()
        if not no_query:
    ...
    def get_occ_labels_attack(self, gt_segmentation, gt_instance, gt_img_is_valid):
        if isinstance(gt_img_is_valid, list):
            gt_segmentation = gt_segmentation[0]
            gt_instance = gt_instance[0]
            gt_img_is_valid = gt_img_is_valid[0]
        gt_segmentation = gt_segmentation[:, :self.n_future+1].long().unsqueeze(2)
