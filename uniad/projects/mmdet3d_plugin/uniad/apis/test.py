...
def custom_multi_gpu_test(model, data_loader, tmpdir=None, gpu_collect=False):
    ...
    for i, data in enumerate(data_loader):
        with torch.no_grad():
        # for tmp in range(1):
            # img_ts = data['img'][0].data[0]    # <class 'torch.Tensor'>
            # print(img_ts)
            # quit()
...
def custom_multi_gpu_attack(model, data_loader, tmpdir=None, gpu_collect=False, corrupted_type=None):
    """Test model with multiple gpus.
    This method tests model with multiple gpus and collects the results
    under two different modes: gpu and cpu modes. By setting 'gpu_collect=True'
    it encodes results to gpu tensors and use gpu communication for results
    ...
    for i, data in enumerate(data_loader):
        # print(data.keys())
        # dict_keys(['img_metas', 'gt_bboxes_3d', 'gt_labels_3d', 'gt_inds', 'img', 'timestamp', 'l2g_r_mat', 'l2g_t', 'gt_fut_traj', 'gt_fut_traj_mask', 'gt_past_traj', 'gt_past_traj_mask', 'gt_sdc_bbox', 'gt_sdc_label', 'gt_sdc_fut_traj', 'gt_sdc_fut_traj_mask', 'gt_lane_labels', 'gt_lane_bboxes', 'gt_lane_masks', 'gt_segmentation', 'gt_instance', 'gt_centerness', 'gt_offset', 'gt_flow', 'gt_backward_flow', 'gt_occ_has_invalid_frame', 'gt_occ_img_is_valid', 'gt_future_boxes', 'gt_future_labels', 'sdc_planning', 'sdc_planning_mask', 'command'])
        # img_ts = data['img'].data[0]    # <class 'torch.Tensor'>
        # print(img_ts)
        # quit()
        result = model(return_loss=False, rescale=True, corrupted_type=corrupted_type, **data)
        # quit()
        with torch.no_grad():
            # EVAL planning
            if eval_planning:
