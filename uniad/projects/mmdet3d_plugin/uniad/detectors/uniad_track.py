...
class UniADTrack(MVXTwoStageDetector):
    ...
    def extract_img_feat(self, img, len_queue=None):
        ...
        # import mmcv
        # mmcv.dump(img, './tmp4/attack_img.pkl')
        img_feats = self.img_backbone(img)
        # mmcv.dump(img_feats, './tmp4/attack_img_feats.pkl')
        # quit()
    ...
    def get_history_bev(self, imgs_queue, img_metas_list):
        ...
        """
        # if self.adv_train and self.pgd_cfg.attack_img:
        #         prev_bev = None
    ...
    def get_bevs(self, imgs, img_metas, prev_img=None, prev_img_metas=None, prev_bev=None):
        ...
        # print(prev_bev.grad_fn)
        img_feats = self.extract_img_feat(img=imgs)
        # print("now grad breakdown, uniad_track.py line 360 ", img_feats[0].grad_fn)
        if self.freeze_bev_encoder:
            if (self.attack): # and self.pgd_cfg.attack_img:      # 得有self.adv_train
                # with torch.no_grad():
                bev_embed, bev_pos = self.pts_bbox_head.get_bev_features(
                        mlvl_feats=img_feats, img_metas=img_metas, prev_bev=prev_bev, freeze=True)
            elif self.adv_train:
                if self.at_attack and (not self.at_train_model):
                # if True:
                    bev_embed, bev_pos = self.pts_bbox_head.get_bev_features(
                            mlvl_feats=img_feats, img_metas=img_metas, prev_bev=prev_bev, freeze=True)
                    # print('\n\nokkkkkkkkkkk\n\n')
                    # quit()
                elif self.at_train_model and (not self.at_attack):
                    with torch.no_grad():
                        bev_embed, bev_pos = self.pts_bbox_head.get_bev_features(
                    ...
                    with torch.no_grad():
                        bev_embed, bev_pos = self.pts_bbox_head.get_bev_features(
                            mlvl_feats=img_feats, img_metas=img_metas, prev_bev=prev_bev)
                    print('errorrrrr uniad_track.py line405')
                    quit()
                    # quit()
            else:
                with torch.no_grad():
            ...
            bev_embed, bev_pos = self.pts_bbox_head.get_bev_features(
                    mlvl_feats=img_feats, img_metas=img_metas, prev_bev=prev_bev)
        # print("now grad breakdown, uniad_track.py line 371 ", bev_embed.grad_fn, bev_pos.grad_fn)
    ...
    def forward_track_train(self,
        ...
        for i in range(num_frame):
            ...
            # print("single single single single")
    ...
    def _forward_attack_single_frame_inference(
        self,
        img,
        img_metas,
        track_instances,
        ...
        # 计算损失
        if self.attack:
            tmp_track_instances = track_instances
            for k in tmp_track_instances._fields.keys():
                # if not tmp_track_instances._fields[k].is_leaf:
                tmp_track_instances._fields[k] = tmp_track_instances._fields[k].clone()
            track_instances_list.append(tmp_track_instances)
            for i in range(nb_dec):
                tmp_track_instances = track_instances_list[i]
                tmp_track_instances.scores = track_scores
                tmp_track_instances.pred_logits = output_classes[i, 0]  # [300, num_cls]
                tmp_track_instances.pred_boxes = output_coords[i, 0]  # [300, box_dim]
                tmp_track_instances.pred_past_trajs = output_past_trajs[i, 0]  # [300,past_steps, 2]     
                out["track_instances"] = tmp_track_instances
                tmp_track_instances, tmp_matched_indices = self.criterion.match_for_single_frame(
                    out, i, if_step=(i == (nb_dec - 1))
                )
        # hard_code: assume the 901 query is sdc query 
        track_instances.obj_idxes[900] = -2
    ...
    def simple_attack_track(
        ...
        """ predict and update """
        prev_bev = self.prev_bev
        frame_res = self._forward_attack_single_frame_inference(
            img,
            img_metas,
            track_instances,
            prev_bev,
        ...
        self.test_track_instances = track_instances
        results = [dict()]
        # 和train少了"track_query_matched_idxes",
        get_keys = ["bev_embed", "bev_pos", 
                    "track_query_embeddings", "track_query_matched_idxes", "track_bbox_results", 
                    "boxes_3d", "scores_3d", "labels_3d", "track_scores", "track_ids"]
        if self.with_motion_head: # 必须有
            get_keys += ["sdc_boxes_3d", "sdc_scores_3d", "sdc_track_scores", "sdc_track_bbox_results", "sdc_embedding"]
        results[0].update({k: frame_res[k] for k in get_keys})
        results = self._det_instances2results(track_instances_fordet, results, img_metas)
        losses = self.criterion.losses_dict
        return losses, results[0]
