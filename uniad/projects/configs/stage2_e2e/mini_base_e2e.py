...
ann_file_train=info_root + f"nuscenes_infos_temporal_train-mini.pkl"
ann_file_val=info_root + f"nuscenes_infos_temporal_val-mini.pkl"
ann_file_test=info_root + f"nuscenes_infos_temporal_val-mini.pkl"
ann_file_attack=info_root + f"nuscenes_infos_temporal_val-mini.pkl"
...
# attack_pipeline = train_pipeline
attack_pipeline_train = [
    dict(type="LoadMultiViewImageFromFilesInCeph", to_float32=True, file_client_args=file_client_args, img_root=''),
    dict(type="NormalizeMultiviewImage", **img_norm_cfg),
    dict(type="PadMultiViewImage", size_divisor=32),
    dict(   # from train
...
data = dict(
    samples_per_gpu=1,      # 应该指的是batch_size
    workers_per_gpu=0,      # 原来是8, 不为0的话enumerate会卡住
    train=dict(
        type=dataset_type,
    ...
    attack_train=dict(
        ...
        test_mode=False,    # !!!
        attack_mode=True,   #  !!!
        ann_file=ann_file_attack,   # !!!!
        pipeline=attack_pipeline_train,    # !!!!
        queue_length=1, # attack_v1必须是1
        patch_size=patch_size,
        canvas_size=canvas_size,
        bev_size=(bev_h_, bev_w_),
        predict_steps=predict_steps,
